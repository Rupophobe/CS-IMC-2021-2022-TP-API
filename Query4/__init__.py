...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    errorMessage = ""
    dataString = "Artist ACTED_IN at least one Film\n"
    try:
        logging.info("Test de connexion avec py2neo...")
        graph = Graph(neo4j_server, auth=(neo4j_user, neo4j_password))
        producers = graph.run("MATCH (n:Artist)-[r:ACTED_IN]->(m:Film) WITH distinct n, count(m) AS titles WHERE titles > 1 RETURN n.nconst, n.primaryName LIMIT 10")
